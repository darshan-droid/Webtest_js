export function createUnityInstance(t, n, c) {
    ...
    m.SystemInfo = function () {
        ...
        m.UnityCache = (h = {
            ...
            ,
            E),
        m.cachedFetch = (w = m.UnityCache,
            y = m.fetchWithProgress,
            S = m.readBodyWithProgress,
            function (o, a) {
                var e, t, i = w.getInstance(), s = T("string" == typeof o ? o : o.url), c = {
                    enabled: (e = s,
                    ...
                    return fetch(n, r).then(function (e) {
                        ...
                        return !c.enabled || c.revalidated ? e : 304 === e.status ? (c.revalidated = !0,
                            i.updateRequestMetaData(c.metaData).then(function () {
                                k("'" + c.metaData.url + "' successfully revalidated and served from the browser cache")
                            }).catch(function (e) {
                                k("'" + c.metaData.url + "' successfully revalidated but not stored in the browser cache due to the error: " + e)
                            }),
                                ...
                                S(e, r.onProgress, r.enableStreamingDownload).then(function (e) {
                                    return c.metaData.size = e.parsedBody.length,
                                        Promise.all([i.storeRequest(n, t), i.updateRequestMetaData(c.metaData)]).then(function () {
                                            k("'" + s + "' successfully downloaded and stored in the browser cache")
                                        }).catch(function (e) {
                                            k("'" + s + "' successfully downloaded but not stored in the browser cache due to the error: " + e)
                                        }),
                                        e
                ...
                return a && (c.control = a.control,
                    ...
                    c.enabled ? i.loadRequest(s).then(function (e) {
                        ...
                        return e ? (n = e.response,
                            ...
                            "immutable" == c.control ? (c.revalidated = !0,
                                i.updateRequestMetaData(r).then(function () {
                                    k("'" + c.metaData.url + "' served from the browser cache without revalidation")
                                }),
                        ...
                        return k("Failed to load '" + c.metaData.url + "' from browser cache due to the error: " + e),
                            y(o, a)
                    }) : y(o, a)
            }
        );
                        ...
                        for (var n in x) {
                            ...
                            if (x[n].hasUnityMarker(i))
                                return s && console.log('You can reduce startup time if you configure your web server to add "Content-Encoding: ' + n + '" response header when serving "' + s + '" file.'),
                                    (r = x[n]).worker || (o = URL.createObjectURL(new Blob(["this.require = ", r.require.toString(), "; this.decompress = ", r.decompress.toString(), "; this.onmessage = ", function (e) {
                                        e = {
                                            id: e.data.id,
            ...
            m.addRunDependency("dataUrl"),
                e.then(function (t) {
                    ...
                    for (n + s > t.length && o("Invalid binary data file header! (pos=" + n + ", headerSize=" + s + ", file length=" + t.length + ")"); n < s;) {
                        var c = r()
                            , d = r()
                            , l = (c + d > t.length && o("Invalid binary data file size! (offset=" + c + ", size=" + d + ", file length=" + t.length + ")"),
                                r())
